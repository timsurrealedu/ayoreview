      ...
      <DashboardHeader
        ...
        actions={
          <button
            ...
            className="flex items-center gap-1.5 px-3.5 py-1.5 rounded bg-action hover:bg-action text-white text-xs font-semibold shadow-lg shadow-action/20 transition active:scale-[0.98]"
          >
      ...
      <main className="p-8 space-y-6 max-w-7xl w-full mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          {filteredLocations.map((loc) => (
            <div
              key={loc.id}
              className="bg-surface border border-line/80 rounded p-5 shadow-sm flex flex-col justify-between hover:border-line transition"
            >
              <div>
                <div className="flex items-start justify-between gap-2 mb-3">
                  <div>
                    <h3 className="font-bold text-ink text-base tracking-tight">
                      {loc.name}
                    </h3>
                  ...
                  <span className="rounded border border-success/25 bg-success-soft px-2 py-1 text-[10px] font-semibold uppercase text-success">
                    {loc.status}
                  </span>
                </div>

                <div className="space-y-2 text-xs text-muted-ink mb-5">
                  <div className="flex items-center gap-2">
                    <MapPin className="w-3.5 h-3.5 text-muted-ink shrink-0" />
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      className="text-ink hover:text-ink truncate hover:underline flex items-center gap-1"
                    >
                      {loc.google_review_url.replace('https://', '')}
                      <ExternalLink className="w-3 h-3 text-muted-ink shrink-0" />
                    </a>
                  </div>
                </div>

                {/* Quick metrics */}
                <div className="grid grid-cols-2 gap-2 p-3 rounded bg-surface/80 border border-line mb-5">
                  <div>
                  ...
                  <div>
                    <div className="text-[10px] text-muted-ink">Kartu Terpasang</div>
        ...
        {showAddModal && (
          <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            <div className="bg-surface border border-line rounded w-full max-w-lg p-6 shadow-2xl space-y-5">
              ...
              <form onSubmit={handleCreate} className="space-y-4 text-xs">
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full bg-surface border border-line rounded px-3 py-2 text-ink focus:outline-none focus:border-success font-mono text-[11px]"
                  />
                  <p className="text-[10px] text-muted-ink mt-1">
