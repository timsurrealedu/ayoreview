...
export const metadata: Metadata = {
  title: 'ReviewTap | Kartu NFC & QR untuk Ulasan Google',
...
function ReviewCard({ priority = false }: { priority?: boolean }) {
  return <div className="review-card"><Image src="/images/review-card-reference.png" alt="Kartu ReviewTap dengan merek Google, nama bisnis, kode QR, dan area ketuk NFC" fill priority={priority} sizes="(max-width: 700px) 88vw, 520px" /></div>;
}
...
export default function LandingPage() {
  return <main lang="id" className={`landing ${archivo.variable}`}>
    ...
    <section className="trust-story" aria-labelledby="trust-title">
      ...
      <div className="friction-note reveal"><p>Masalahnya sederhana</p><strong>Pelanggan puas sering lupa memberi ulasan.</strong><span>ReviewTap mengingatkan mereka di waktu yang tepat, sebelum meninggalkan tempat Anda.</span></div>
    </section>
    ...
    <section className="closing" aria-labelledby="closing-title"><div><ReviewStars label="Lima bintang Ulasan Google" /><h2 id="closing-title">Ubah kepuasan pelanggan menjadi ulasan yang terlihat.</h2><p>Pasang ReviewTap di bisnis Anda dan mulai meminta ulasan dengan cara yang lebih mudah.</p></div><Link href="/signup" className="button button-inverse">Beli Sekarang <ArrowRight aria-hidden="true" /></Link></section>
    <footer><Link href="/" className="wordmark"><span aria-hidden="true">R</span>ReviewTap</Link><p>Kartu NFC + QR untuk Ulasan Google.</p><div><Link href="/login">Masuk</Link><Link href="/signup">Beli Sekarang</Link></div><small>© 2026 ReviewTap.</small></footer>
  </main>;
}
