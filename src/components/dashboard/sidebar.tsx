...
export function DashboardSidebar({ isPlatformAdmin, shell = 'merchant', userName, userEmail }: { isPlatformAdmin?: boolean; shell?: Shell; userName?: string | null; userEmail?: string }) {
  ...
  return <><a href="#main-content" className="fixed left-3 top-3 z-[70] -translate-y-20 rounded bg-action px-4 py-3 text-sm font-bold text-white focus:translate-y-0">Lewati ke konten utama</a><header className="sticky top-0 z-40 flex min-h-16 items-center justify-between border-b border-line bg-surface px-4 md:hidden"><Link href={home} className="flex min-h-11 items-center gap-2"><Logo size={32} className="shrink-0" /><span className="text-sm font-bold text-ink">{title}</span></Link><button ref={triggerRef} type="button" onClick={() => setOpen(true)} aria-label="Buka navigasi" aria-expanded={open} aria-controls={`${shell}-navigation`} className="flex h-11 w-11 items-center justify-center rounded border border-line text-ink"><Menu className="h-5 w-5" /></button></header><aside className="admin-sidebar no-print hidden min-h-screen w-64 shrink-0 border-r border-line md:block">{panel}</aside>{open && <div className="fixed inset-0 z-50 md:hidden"><button type="button" className="absolute inset-0 bg-ink/35" onClick={() => setOpen(false)} aria-label="Tutup navigasi" /><aside id={`${shell}-navigation`} role="dialog" aria-modal="true" aria-label={subtitle} className="relative h-full w-[min(20rem,88vw)] shadow-2xl">{panel}</aside></div>}</>;
}
